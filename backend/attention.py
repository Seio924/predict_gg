...
class Attention():
  ...
  def fit(self, x, y):
    ...
    _inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, 500, 177], name='inputs')
    ...
    if self.task == 'classification':
      ...
      correct_prediction = tf.equal(tf.round(outputs), Y)
    ...
    # 가중치를 저장하고 불러오는 Saver 객체 생성
    saver = tf.train.Saver()

    if os.path.exists("C:/GitHub/predict_gg/backend/ATTENTION_checkpoint"):
        saver.restore(sess, "C:/GitHub/predict_gg/backend/ATTENTION_checkpoint/best_model.h5")
    ...
    for i in range(iterations):
      ...
      Input = x[idx,:,:]            
      _, step_loss, step_accuracy, step_mse = sess.run([train, loss, accuracy, mse], 
      ...
      if i % iteration_per_epoch == iteration_per_epoch-1:
        print('Epoch: ' + str(int(i/iteration_per_epoch)) + 
              ', Loss: ' + str(np.round(step_loss, 4)) + ', Accuracy: ' + str(np.round(step_accuracy, 4)) + ', MSE: ' + str(np.round(step_mse, 4)))
    
      if i % iteration_per_epoch == iteration_per_epoch-1 and np.round(step_loss, 4) < min_loss:
        min_loss = np.round(step_loss, 4)
        saver.save(sess, "C:/GitHub/predict_gg/backend/ATTENTION_checkpoint/best_model.h5")
        print("model save")
