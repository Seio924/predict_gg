...
class LoadData():
    ...
    def get_summoner_invertal_list(self, summoner_start, num_matches):
        ...
        for summoner_name in summonerId:
            ...
            if num == num_matches:
                with open("api_data/api_interval_list1.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
                    for item in data_list:
                        file.write(str(item) + "\n")

                with open("api_data/api_win_lose_list1.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
                    for item in win_lose_list:
                        file.write(str(item) + "\n")       
                break

        with open("api_data/api_interval_list1-4.txt", "w") as file:
            # 리스트의 각 요소를 파일에 쓰기
            for item in data_list:
                file.write(str(item) + "\n")

        with open("api_data/api_win_lose_list1-4.txt", "w") as file:
            # 리스트의 각 요소를 파일에 쓰기
            for item in win_lose_list:
                file.write(str(item) + "\n")   

if __name__ == "__main__":
    load_data_instance = LoadData(api_key='RGAPI-5be03e61-55d1-491e-82aa-072eecff09c0') #수빈api
