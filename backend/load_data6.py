...
class LoadData():
    ...
    def get_summoner_invertal_list(self, summoner_start, num_matches):
        ...
        for summoner_name in summonerId:
            ...
            if num == num_matches:
                with open("api_data/api_interval_list6.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
                    for item in data_list:
                        file.write(str(item) + "\n")

                with open("api_data/api_win_lose_list6.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
...
if __name__ == "__main__":
    load_data_instance = LoadData(api_key='RGAPI-0527ec35-a442-4759-88e9-0db76a4ac187') #재원api
