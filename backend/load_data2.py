...
class LoadData():
    ...
    def get_match_data(self, matchid):
        ...
        with open('api_data/api_match_info2.json', 'w', encoding='utf-8') as json_file:
    ...
    def get_timeline_data(self, matchid):
        ...
        with open('api_data/api_timeline_info2.json', 'w', encoding='utf-8') as json_file:
    ...
    def get_summoner_invertal_list(self, summoner_start, num_matches):
        ...
        for summoner_name in summonerId:
            ...
            for match_id in match_ids:
                ...
                with open('api_data/api_match_info2.json', 'r', encoding="utf-8") as f:
                    match_info = json.load(f)

                with open('api_data/api_timeline_info2.json', 'r', encoding="utf-8") as f:
                ...
                test = PreprocessData('./api_data/api_match_info2.json', './api_data/api_timeline_info2.json')
            ...
            if num == num_matches:
                with open("api_data/api_interval_list2.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
                    for item in data_list:
                        file.write(str(item) + "\n")

                with open("api_data/api_win_lose_list2.txt", "w") as file:
                    # 리스트의 각 요소를 파일에 쓰기
                    for item in win_lose_list:
                        file.write(str(item) + "\n")       
                break

        with open("api_data/api_interval_list2-4.txt", "w") as file:
            # 리스트의 각 요소를 파일에 쓰기
            for item in data_list:
                file.write(str(item) + "\n")

        with open("api_data/api_win_lose_list2-4.txt", "w") as file:
            # 리스트의 각 요소를 파일에 쓰기
            for item in win_lose_list:
                file.write(str(item) + "\n")       

if __name__ == "__main__":
    load_data_instance = LoadData(api_key='RGAPI-0bb288d2-0eb1-484a-b95b-ccedf284757c') #원빈api
    #load_data_instance.get_summoner_invertal_list(summoner_start=125, num_matches=2500)
    #load_data_instance.get_summoner_invertal_list(summoner_start=875, num_matches=2500)
    #load_data_instance.get_summoner_invertal_list(summoner_start=1625, num_matches=2500) 
    load_data_instance.get_summoner_invertal_list(summoner_start=2375, num_matches=2500) 
